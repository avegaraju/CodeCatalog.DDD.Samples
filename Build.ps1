...
.PARAMETER Version
The version of the build to publish
...
if (!(Test-Path $NUGET_EXE)) {
    ...
    $existingPaths = $Env:Path -Split ';'| ? { $_ } | Where-Object { Test-Path $_ }
...
Invoke-Expression "& `"$CAKE_EXE`" `"$Script`" -target=`"$Target`" -packVersion=`"$Version`" -configuration=`"$Configuration`" -verbosity=`"$Verbosity`" $UseMono $UseDryRun $UseExperimental $ScriptArgs"
exit $LASTEXITCODE